...
func (t *SimpleChaincode) initlicense(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// ==== Input sanitation ====
	fmt.Println("- start init license")
	if len(args[0]) <= 0 {
	...
	licenseAsBytes, err := stub.GetState(hash)
	if err != nil {
		return shim.Error("Failed to get license: " + err.Error())
	} else if licenseAsBytes != nil {
		fmt.Println("This license already exists: " + hash)
		return shim.Error("This license already exists: " + hash)
	}

	// ==== Create license object and marshal to JSON ====
	objectType := "license"
	license := &license{objectType, hash, name, owner}
	licenseJSONasBytes, err := json.Marshal(license)
	if err != nil {
		return shim.Error(err.Error())
	}
	//Alternatively, build the marble json string manually if you don't want to use struct marshalling
	//licenseJSONasString := `{"docType":"Marble",  "name": "` + marbleName + `", "color": "` + color + `", "size": ` + strconv.Itoa(size) + `, "owner": "` + owner + `"}`
	//licenseJSONasBytes := []byte(str)

	// === Save marble to state ===
	err = stub.PutState(hash, licenseJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	indexName := "file~name"
	licenseIndexKey, err := stub.CreateCompositeKey(indexName, []string{license.Hash})
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	value := []byte{0x00}
	stub.PutState(licenseIndexKey, value)
...
func (t *SimpleChaincode) readMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	hash = args[0]
	valAsbytes, err := stub.GetState(hash) //get the marble from chaincode state
	if err != nil {
		jsonResp = "{\"Error\":\"Failed to get state for " + hash + "\"}"
		return shim.Error(jsonResp)
	} else if valAsbytes == nil {
		jsonResp = "{\"Error\":\"Marble does not exist: " + hash + "\"}"
		return shim.Error(jsonResp)
	}
